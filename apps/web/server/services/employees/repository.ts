/**
 * Employees repository — Nitro-versie.
 *
 * Kopie van `apps/api/src/modules/employees/repository.ts` met identieke logica.
 * Import-paden aangepast voor Nitro-context.
 *
 * DE enige laag die Prisma aanraakt voor employees.
 * Elke functie wrapped in `withTenant` → RLS actief, PII-key beschikbaar.
 */

import { withTenant, type Prisma } from '../../utils/prisma.js'
import type {
  CreateEmployeeInput,
  EmployeeListQuery,
  UpdateEmployeeInput,
  ...
  return withTenant(tenantId, async (tx) => {
    ...
    if (rbac?.employeeIdFilter) {
      ...
      if (callerEmployee?.employeeId) {
        rbacWhere = {
          OR: [
            { id: callerEmployee.employeeId },
            { managerId: callerEmployee.employeeId },
          ],
        }
      } else {
